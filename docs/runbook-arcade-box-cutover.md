...
cores for every CI build. A new `homelab.tiers.cpusetFence` (default true,
so ac-box is unchanged) turns `AllowedCPUs` off per host; `CPUWeight` and
`MemoryMax` still apply, which is what a runaway build needs to hit.
...
**0.1 Key access.** Once, typing the password:

```bash
ssh-copy-id -i ~/.ssh/id_ed25519_ac-host.pub root@192.168.1.218
ssh -o BatchMode=yes root@192.168.1.218 true && echo key-ok
```
...
| `modules/platform/secrets.nix` | `BUILDKITE_AGENT_NAME=${homelab.host.name}` in the `ci-env` template | two agents named `ac-box` would be indistinguishable in Buildkite |
| `.sops.yaml` + `secrets/ac-box.yaml` | add `&arcade-box` (section 1's recipient); `SOPS_AGE_KEY="$(ssh-to-age -private-key -i ~/.ssh/id_ed25519_ac-host)" sops updatekeys secrets/ac-box.yaml` | D4; the box decrypts with its host key at activation |
...
| --- | --- | --- | --- |
| critical | 0.20 (~6.4 GiB) | 0.50 | the lobby containers, sidecars, bot via `cgroup_parent` (peak 2.5 GiB on ac-box) |
| interactive | 0.20 (~6.4 GiB) | 0.20 | arcade (Mindustry `-Xmx1G`), samba, rsync, observability (~1.3 GiB peak) |
| background | 0.00 | 0.00 | nothing -- no tenant of this tier on this host |
| batch | 0.45 (~14 GiB) | 0.30 | the agent, MinIO, `nix-daemon` (`modules/platform/nix.nix` slices it here) |

Sum 0.85 of the 0.9 budget. Weights only, no `AllowedCPUs` (D6): a CI
build takes the whole box when the lobbies are idle and yields 500:300
when they are not. `--spawn` stays 1 until `homelab-bfq.5`-style numbers
exist for this host.
...
| Fact | Value | Source |
| --- | --- | --- |
| CPU model, cores, threads | | `lscpu`, phase 1 |
| Memory (GiB, `/proc/meminfo`) | | phase 1 |
| Disk, layout, filesystem | | `lsblk`, phase 1 |
| LAN NIC name | | `ip -br link`, phase 1 |
| `system.stateVersion` | | `/etc/nixos/configuration.nix`, phase 1 |
