...
| 33 | **homelab's Buildkite pipeline has never run.** The only `queue=self` agent (`ac-host-ci-agent-1`, up since 12 Sep 12:37) has executed 32 jobs, every one of them `ac-host`: none for the homelab pushes of 12 Sep evening, none for `eb55a83` (13 Sep 01:25), and none for `home-arcade` `3ae0a71` either. `.buildkite/pipeline.yml` is in git (bead `.29`, "verified locally") but the pipeline *object* — repo, webhook, cluster, first step `buildkite-agent pipeline upload` — is a Buildkite UI action (`home-arcade/docs/ci.md` "First-time pipeline") and nothing records it being done for homelab. Without it `queue-closure` never writes, and row 29's "push → 03:30 next night" cannot happen. | `bash scripts/hub-pipeline.sh homelab` and `… home-arcade` (row 25): the objects come from `hub/pipelines/*.json`, unclustered, `queue: self`, GitHub builds on push. Then push anything to homelab and watch the agent log for a `homelab/builds/1` job; if none comes, the GitHub App does not cover the repo and the script has printed the webhook to add by hand. | `homelab-bqo.50` — **Open, one command from done.** Found 13 Sep by waiting for the build that the push should have produced; confirmed by the operator the same night: no homelab pipeline exists in Buildkite. Row 25's "a misclick could detach a pipeline" was optimistic — the closure's pipeline was never attached. The script is proven by `--dry-run` for both trees and by a read-only token check (`GET /v2/access-token`); the run itself is the supervisor's, and the proof is the first homelab build on the `ac-box` agent, not the 201. |
