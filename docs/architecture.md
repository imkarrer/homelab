...
| 23 | Seven secrets still hand-placed | sops, same shape as `arcade-smb-password` | `homelab-bqo.39` — **Open**, unblocked by `a3375be`. Do them after tonight's activation proves the first one. `/var/lib/ac-host/.env` and `.env.buildkite` are `sops.templates` (secret keys + literal config). |
...
| 25 | Buildkite pipeline *objects* (which repo, branch, steps file) live in the Buildkite UI; only the steps are in git | Pipeline-as-code via the Buildkite API or Terraform provider | `homelab-bqo.41` — **Open.** Low urgency; a UI misclick could silently detach a pipeline from its steps file. |
