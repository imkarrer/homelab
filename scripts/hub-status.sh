  ...
  if [ -z "$BOXTXT" ] || [ -z "$SYS" ]; then
    ...
    closure_behind() {
      if [ "$CLTIMER" != "enabled" ]; then
        ...
        # 4.2 moves the agent; the Z840 from then on). Behind, and only a hand
        # switch moves it.
        note "system closure is $1 and nothing is staged - no CI agent on this host (ci is not in its inventory), so nothing stages a closure here; only a hand switch moves it (ADR 0010: the Z840 after the cutover, arcade-box before it); until then only a hand switch moves it (runbook 5.4)"
      else
