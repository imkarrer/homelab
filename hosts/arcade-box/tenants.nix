...

{
  homelab.tenants = {
    ...
    # (hosts/arcade-box/tenants/arcade.nix, the host side).
    arcade = {
      ...
      # (hosts/arcade-box/tenants/arcade.nix's services.samba / services.rsyncd
    ...
    observability = {
      ...
      # Not the derived default (/var/lib/observability): nothing has ever
      # lived there. The state is where the nixpkgs modules keep it, under the
      # accounts they run as -- verified on the box 16 Sep 2026 (`systemctl
      # show`, `stat`); neither is a DynamicUser indirection under
      # /var/lib/private.
      #
      #   /var/lib/grafana      grafana:grafana 0700. User=grafana, no
      #                         StateDirectory. data/grafana.db is the sqlite
      #                         database: dashboards, users, annotations,
      #                         alert state; plugins/ beside it.
      #   /var/lib/prometheus2  prometheus:prometheus 0700. StateDirectory=
      #                         prometheus2. The TSDB: 14 d of samples, per
      #                         services.prometheus.retentionTime.
      #
      # Until 16 Sep 2026 this declared only /var/lib/monitoring, so the
      # backup -- scripts/hub-backup.sh reads this list and nothing else --
      # faithfully pulled 12 K of secrets while the 170 M that would hurt to
      # lose went uncopied (homelab-bqo.58). /var/lib/monitoring is not state
      # any more: since homelab-bqo.39.3 its four files are symlinks into
      # /run/secrets, installed by modules/platform/secrets.nix from the sops
      # file in git; a backup of it would hold four dangling links. The
      # directory itself stays (modules/observability creates it, secrets.nix
      # installs into it) -- it is only no longer declared, because `dirs`
      # means "what gets backed up" and nothing else.
      #
      # Nothing else, on the box either: state.dirs has one reader, the
      # backup script's nix eval. No tmpfiles rule, chown or ReadWritePaths
      # is derived from it, which is what makes naming a directory owned by
      # another account safe here. Keep it that way -- a chown to a tenant
      # account would take grafana's data away from grafana at the next
      # switch.
      state = {
        dirs = [
          "/var/lib/grafana"
